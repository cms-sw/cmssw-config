...
sub updateEnvVarMK
{
  ...
  foreach my $key (keys %$env)
  {
    ...
    if($key=~/^SCRAM_(BUILDVERBOSE|NOPLUGINREFRESH|NOSYMCHECK|NOLOADCHECK|TOOL_HOME|VERSION|LOOKUPDB|SYMLINKS|TEST_RUNNER_PREFIX|NOEDM_CHECKS)$/){next;}
...
sub src2store_copy()
{
  ...
  for my $f ("SKIP_FILES", "INSTALL_SCRIPTS")
  {
    my $flag=$core->flags($f);
    if($flag ne ""){print $fh "${safepath}_${f} := $flag\n";}
  }
